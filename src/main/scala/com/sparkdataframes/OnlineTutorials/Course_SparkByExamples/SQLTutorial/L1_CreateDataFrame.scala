...
//	 */
//	def usingSessionCreateFataFrameOnSequenceOfRowsWithSchema[T](spark: SparkSession,
//												   seq: Seq[T], //Seq[(String, String)],
//												   colnames: Seq[String]): DataFrame = {
//
//		// NOTE: T is a type for tuples where T = (A, B, C, D ... ) ==> seq contains tuples as its rows
//
//		//val seqOfRows: Seq[Row] = seq.map { case (name1, name2) => Row(name1, name2) }
//		usingSessionCreateFataFrameOnSequenceOfRowsWithSchema(spark, GeneralUtils.seqToRows(seq), colnames)
//
//	}
...
//	 */
//	def usingSessionCreateDataFrameOnRowRDDAndSchema[T](spark: SparkSession,
//										    rdd: RDD[T], //RDD[(String, String)],
//										    colnames: Seq[String]): DataFrame = {
