...
//
//			val ctry: Column = col(Country.enumSimpleName)
//
//			// Excluding countries that belong in multiple hemispheres, leaving that case to 'otherwise'
//			// WARNING: use :_*      ctry.isin(xs:_*)
//			val conditionSouth: Column = ctry isin (SOUTHERN_HEMI.map(_.enumSimpleName)) and ! (ctry isin(countriesFromOtherHemis(SH)) )
//			val conditionNorth: Column = ctry isin (NORTHERN_HEMI.map(_.enumSimpleName)) and ! (ctry isin(countriesFromOtherHemis(NH)) )
//			val conditionEast: Column = ctry isin (EASTERN_HEMI.map(_.enumSimpleName)) and ! (ctry isin(countriesFromOtherHemis(EH)) )
//			val conditionWest: Column = ctry isin (WESTERN_HEMI.map(_.enumSimpleName)) and ! (ctry isin(countriesFromOtherHemis(WH)) )
//			val conditionCentral: Column = ctry isin (CENTRAL_HEMI.map(_.enumSimpleName)) and ! (ctry isin(countriesFromOtherHemis(CH)) )
//
...
//				when(conditionSouth, Hemisphere.SouthernHemisphere.enumSimpleName)
//					.when(conditionNorth, Hemisphere.NorthernHemisphere.enumSimpleName)
//					.when(conditionEast, Hemisphere.EasternHemisphere.enumSimpleName)
//					.when(conditionWest, Hemisphere.WesternHemisphere.enumSimpleName)
//					.when(conditionCentral, Hemisphere.CentralHemisphere.enumSimpleName)
//			)
//
//
