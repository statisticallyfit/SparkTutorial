...
object L16_Joins extends AnyFunSpec with Matchers {
	...
	case class OuterJoinSpecs[LEFT: TypeTag, RIGHT: TypeTag, TARGET: TypeTag](leftDF: DataFrame,
		...
		def testMismatchedRowsForOuterJoin = {
			...
			// lc = left col
			val lc: List[Option[TARGET]] = getColAs_complicated[TARGET](leftDF, leftColname)
			val rc: List[Option[TARGET]] = getColAs_complicated[TARGET](rightDF, rightColname)
		...
		def testDifferingRecordsHaveNullsInOuterJoin = {
			val leftCol_OJ: List[Option[TARGET]] = getColAs_complicated[TARGET](outerJoin, leftColname)
			val rightCol_OJ: List[Option[TARGET]] = getColAs_complicated[TARGET](outerJoin, rightColname)
	...
	case class LeftOuterJoinSpecs[LEFT: TypeTag, RIGHT: TypeTag, TARGET: TypeTag](leftDF: DataFrame,
		...
		def testDifferingRecordsHaveNullsInLeftOuterJoin = {
			val leftCol_LOJ: List[Option[TARGET]] = getColAs_complicated[TARGET](leftOuterJoin, leftColname)
			val rightCol_LOJ: List[Option[TARGET]] = getColAs_complicated[TARGET](leftOuterJoin, rightColname)
	...
	case class RightOuterJoinSpecs[LEFT: TypeTag, RIGHT: TypeTag, TARGET: TypeTag](leftDF: DataFrame,
		...
		def testDifferingRecordsHaveNullsInRightOuterJoin = {
			val leftColRightOuter: List[Option[TARGET]] = getColAs_complicated[TARGET](rightOuterJoin, leftColname)
			val rightColRightOuter: List[Option[TARGET]] = getColAs_complicated[TARGET](rightOuterJoin, rightColname)
		...
		def testMatchingRecordsDontHaveNullsInRightOuterJoin = {
			// NOTE: converting the left df col to be of type RIGHT (Int) since rightdf (deptdf) col is of type Integer
			//  while leftdf (empdf) col is of type String
			val leftCol_ROJ: List[Option[TARGET]] = getColAs_complicated[TARGET](rightOuterJoin, leftColname)
			val rightCol_ROJ: List[Option[TARGET]] = getColAs_complicated[TARGET](rightOuterJoin, rightColname)
	...
	case class LeftSemiJoinSpecs[LEFT: TypeTag, RIGHT: TypeTag, TARGET: TypeTag](leftDF: DataFrame,
		...
		def testIntersectedColumnsForLeftSemiJoin = {
			...
			val leftCol_LSJ: List[Option[TARGET]] = getColAs_complicated[TARGET](leftSemiJoin, leftColname)
	...
	case class LeftAntiJoinSpecs[LEFT: TypeTag, RIGHT: TypeTag, TARGET: TypeTag](leftDF: DataFrame,
		...
		def testIntersectedColumnsForLeftAntiJoin = {
			...
			val leftCol_LAJ: List[Option[TARGET]] = getColAs_complicated[TARGET](leftAntiJoin, leftColname)
			val leftCol_LSJ: List[Option[TARGET]] = getColAs_complicated[TARGET](leftSemiJoin, leftColname)
			//val rightCol_LSJ: List[Option[TARGET]] = getColAs[TARGET](leftSemiJoin, rightColname)
