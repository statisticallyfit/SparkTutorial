...
import com.DocumentingSparkByTestScenarios.CreatingDataFrames.{Art, Literature, Painter}
...
object CreatingDataFrames {
	...
	object Musical extends Enumeration with Instrument[Musical.type] {
		type Instrument = Value
		val Flute, Oboe, Clarinet, Trombone, Tuba, FrenchHorn, Violin, Harp, Trumpet, Saxophone, Harmonica, Xylophone, Mandolin, Piano, Guitar, Cello = Value
	}
	...
	object Art extends Enumeration {
		type Type = Value
		val Painting, Music, Literature, Sculpture, Architecture, Theatre = Value
	}

	object Writer extends Enumeration with Artist[Art.Value] {
		type Author = Value
		val AlfredLordTennyson, LordByron, EdgarAllanPoe, EmilyDickinson, JulesVerne, JaneAusten, TSEliot, HansChristianAnderson, MarkTwain, JamesJoyce, LeoTolstoy, FScottFitzgerald, GeorgeOrwell, HermanMelville, RoaldDahl, AgathaChristie, WilliamShakespeare, AlexandreDumas, ErnestHemingway, HermanHesse, AntonChekhov, AlexanderPushkin, GeoffreyChaucer = Value
	}
	object Painter extends Enumeration with Artist[Art.Painting.type] {
		type Author = Value
	...
	val domainToAuthor: Map[Art.Value, Painter.Value] = Map(
		Art.Painting -> Painter.VanGogh,
